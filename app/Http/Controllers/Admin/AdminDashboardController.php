...
class AdminDashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // If no there in group we get the all tickets
            ...
            $user=Auth::user();
            $category_ids=json_decode($user->categories) ?? [];
            $projects_ids=json_decode($user->projects) ?? [];
            $products_ids=json_decode($user->products) ?? [];
            $sub_categories_ids=json_decode($user->sub_categories) ?? [];

            $query=Ticket::query();
            $query->leftJoin('projects','tickets.project','projects.id');

            if(!empty($category_ids)){
                $query->whereIn('category_id',$category_ids);
            }
            if(!empty($sub_categories_ids)){
                $query->whereIn('subcategory',$sub_categories_ids);
            }
            if(!empty($products_ids)){
                $query->whereIn('projects.product_id',$products_ids);
            }
            if(!empty($projects_ids)){
                $query->whereIn('project',$projects_ids);
            }
            $tickets  = $query->get();
            $active = $query->whereIn('status', ['New', 'Re-Open','Inprogress'])->get();
            $closed = $query->where('status', 'Closed')->get();
            $onhold = $query->where('status', 'On-Hold')->get();
            $myticket = Ticket::where('user_id', auth()->id())->get();
            $myassigned = Ticket::where('toassignuser_id', Auth::id())->get();
            $overdue = $query->whereIn('overduestatus', ['Overdue'])->get();
            $assigned = Ticket::where('myassignuser_id', Auth::id())->get();
            ...
            if(request()->ajax()) {
                ...
                $data = $query->latest('tickets.updated_at')->get();
                return DataTables::of($data)
