  ...
  public function showLoginForm()
    ...
    public function login(Request $request)
    {
        if(setting('CAPTCHATYPE') == 'off'){
            $request->validate([
                'username'     => 'required|exists:customers|max:255',
        ...
        $customerExist = Customer::where(['username' => $request->username, 'status' => 0])->exists();
        
        if ($customerExist) {
            return redirect()->back()->with('error',trans('langconvert.functions.customerinactive'));
        }
        
        $unverifiedCustomer = Customer::where('username', $request->username)->first();
        ...
        return back()->withInput()->withErrors(['username' => trans('langconvert.functions.invalidusernamepass')]);
    ...
    public function ajaxlogin(Request $request){
        ...
        if (Auth::guard('customer')->attempt(array('username' => $user, 'password' => $pass)))
        {
            $cust = Customer::find(Auth::guard('customer')->id());
            $cust->update([
                'last_login_at' => Carbon::now()->toDateTimeString(),
