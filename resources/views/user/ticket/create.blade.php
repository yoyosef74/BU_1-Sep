						...
						<form method="POST" id="user_form" enctype="multipart/form-data">
							...
							<div class="card-body">
								<div class="form-group ">
									<div class="row">
										<div class="col-md-3">
											<label
												class="form-label mb-0 mt-2">{{trans('langconvert.admindashboard.ticketsubject')}}
								...
								<div class="form-group">
									<div class="row">
										<div class="col-md-3">
											<label
												class="form-label mb-0 mt-2">{{trans('langconvert.admindashboard.ticketproject')}}
												<span class="text-red">*</span></label>
										</div>
										<div class="col-md-9">
											<select
												class="form-control select2-show-search  select2 @error('project') is-invalid @enderror"
												data-placeholder="Select Project" name="project" id="project">
												@if(count($projects) > 1)
												<option label="Select Project"></option>
												@endif
												@foreach ($projects as $project)
												<option value="{{ $project->id }}" @if(old('project')) selected @endif
													data-product_id="{{ $project->product_id }}">{{ $project->name }}
												</option>
												@endforeach
								...
								<div class="form-group">
									<div class="row">
										<div class="col-md-3">
											<label
												class="form-label mb-0 mt-2">{{trans('langconvert.admindashboard.ticketcategory')}}
												<span class="text-red">*</span></label>
										</div>
										<div class="col-md-9">
											<select
												...
												<option value="{{ $category->id }}" @if(old('category')) selected @endif
													data-product_id="{{$category->product_id}}">{{ $category->name }}
												</option>
												@endforeach
								...
								<div class="form-group" id="selectssSubCategory" style="display: none;">
									...
									<div class="row">
										<div class="col-md-3">
											<label
												class="form-label mb-0 mt-2">{{trans('langconvert.newwordslang.ticketsubcategory')}}</label>
										</div>
										<div class="col-md-9">
											<select class="form-control select2-show-search select2"
												data-placeholder="Select SubCategory" name="subscategory"
												id="subscategory">
								...
								<div class="form-group ticket-summernote ">
									<div class="row">
										<div class="col-md-3">
											<label
												class="form-label mb-0 mt-2">{{trans('langconvert.admindashboard.ticketdescription')}}
												<span class="text-red">*</span></label>
										</div>
										<div class="col-md-9">
											<textarea
								...
								<div class="form-group">
									<div class="row">
										<div class="col-md-3">
											<label
												class="form-label mb-0 mt-2">{{trans('langconvert.admindashboard.uploadimage')}}</label>
										</div>
										<div class="col-md-9">
							...
							<div class="card-footer">
								<div class="form-group float-end">
									<input type="submit" class="btn btn-secondary btn-lg purchasecode ticket_create_btn"
										value="Create Ticket">
								</div>
							</div>
						</form>
...
<script type="text/javascript">
	"use strict";
			...
			(function($){
				...
				var product;
                   if($('#project').val() != ''){
	                product=$('#project option:selected').data('product_id');
					$('#category option').attr('disabled','disabled')
					$('#category option[data-product_id="'+product+'"]').prop('disabled', false);
					$('#category').select2();
}
				$('#project').on('change',function(e) {
					product=$(this).find(':selected').data('product_id');
					$("#subscategory,#category").val('').trigger('change');
					$('#category option').attr('disabled','disabled')
					$('#category option[data-product_id="'+product+'"]').prop('disabled', false);
					$('#category').select2();
				});
				...
				$('body').on('submit', '#user_form', function (e) {
					e.preventDefault();
					$(".ticket_create_btn").prop('disabled',true);
					$('#SubjectError').html('');
					$('#MessageError').html('');
					$('#EmailError').html('');
					$('#CategoryError').html('');
					...
					$.ajax({
						...
						error: function(data){
							$(".ticket_create_btn").prop('disabled',false);
