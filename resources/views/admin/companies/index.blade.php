@extends('layouts.adminmaster')
		...
		@endsection
							...
							<div class="page-header d-xl-flex d-block">
								<div class="page-leftheader">
									<h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{trans('langconvert.adminmenu.companies')}}</span></h4>
								</div>
							</div>
							<!--End Page header-->
							...
							<div class="col-xl-12 col-lg-12 col-md-12">
								<div class="card ">
									<div class="card-header border-0 d-md-max-block">
										<h4 class="card-title mb-md-max-2">{{trans('langconvert.adminmenu.companies')}}</h4>
										<div class="card-options d-md-max-block">
											@can('Project Create')
											
											<a href="javascript:void(0)" class="btn btn-success me-3 mb-md-max-2 mw-10" id="create-new-projects">{{trans('langconvert.admindashboard.addcompanies')}}</a>
											@endcan
    ...
    dropdownParent: $("#addtestimonial")
				...
				$('#create-new-projects').on('click', function () {
					...
					$('.modal-title').html("{{trans('langconvert.admindashboard.addnewcompany')}}");
				...
				$('body').on('click', '.edit-testimonial', function () {
					...
					$.get('companies/' + company_id , function (data) {
						$('#nameError').html('');
						$('.modal-title').html("{{trans('langconvert.admindashboard.editcompany')}}");
					...
					var _id = $(this).data("id");
						...
						.then((willDelete) => {
							if (willDelete) {
								$.ajax({
									type: "get",
									url: SITEURL + "/admin/companies/delete/"+_id,
				...
				$('body').on('click', '#massdelete', function () {
					...
					if(id.length > 0){
						...
						.then((willDelete) => {
							if (willDelete) {
								$.ajax({
									url:"{{ url('admin/masscompany/delete')}}",
									method:"GET",
									data:{id:id},
									success:function(data)
									{
				...
				$('body').on('submit', '#projects_form', function (e) {
					...
					$.ajax({
						type:'POST',
						url: SITEURL + "/admin/companies/create",
						data: formData,
						cache:false,
						contentType: false,
						processData: false,
