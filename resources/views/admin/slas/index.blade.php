...
<div class="page-header d-xl-flex d-block">
	<div class="page-leftheader">
		<h4 class="page-title"><span
				class="font-weight-normal text-muted ms-2">{{trans('langconvert.adminmenu.sla')}}</span></h4>
	</div>
</div>
<!--End Page header-->
...
<div class="col-xl-12 col-lg-12 col-md-12">
	<div class="card ">
		<div class="card-header border-0 d-md-max-block">
			<h4 class="card-title mb-md-max-2">{{trans('langconvert.adminmenu.sla')}}</h4>
			<div class="card-options d-md-max-block">
				@can('Project Create')

				<a href="javascript:void(0)" class="btn btn-success me-3 mb-md-max-2 mw-10"
					id="create-new-projects">{{trans('langconvert.admindashboard.addsla')}}</a>
				@endcan
		...
		<div class="card-body">
			<div class="table-responsive spruko-delete">
				...
				<button id="massdelete" class="btn btn-outline-light btn-sm mb-4 data-table-btn"><i
				...
				<table class="table table-vcenter text-nowrap table-bordered table-striped ticketdeleterow w-100"
					...
					<thead>
						<tr>
							...
							<th>{{trans('langconvert.admindashboard.tickets_monthly')}}</th>
			...
			(function($)  {
				...
				$('#support-articlelists').DataTable({
					...
					columns: [
						...
						{ data: 'name', name: 'name' },
						{ data: 'tickets_monthly', name: 'tickets_monthly' },
				...
				$('#create-new-projects').on('click', function () {
					...
					$('.modal-title').html("{{trans('langconvert.admindashboard.addnewsla')}}");
				...
				$('body').on('click', '.edit-testimonial', function () {
					var sla_id = $(this).data('id');
					$.get('sla/' + sla_id , function (data) {
						$('#nameError').html('');
						$('.modal-title').html("{{trans('langconvert.admindashboard.editsla')}}");
						$('#btnsave').val("edit-project");
						$('#addtestimonial').modal('show');
						$('#projects_id').val(data.id);
						$('#name').val(data.name);
						$('#tickets_monthly').val(data.tickets_monthly);
					...
					var _id = $(this).data("id");
						...
						.then((willDelete) => {
							if (willDelete) {
								$.ajax({
									type: "get",
									url: SITEURL + "/admin/sla/delete/"+_id,
				...
				$('body').on('submit', '#projects_form', function (e) {
					...
					$.ajax({
						...
						error: function(data){
							$('#nameError').html('');
							toastr.error('SLA Name is Already Exists');
							$('#btnsave').html('Save Changes');
						}
					});
